...
export function EditPartyDialog({ party, open, onOpenChange, onPartyUpdate }: EditPartyDialogProps) {
  const form = useForm<EditPartyFormValues>({
    ...
    defaultValues: {
      ...
      place: party.place,
      menu: party.menu.map(item => ({ value: item })),
  ...
  const onSubmit = (values: EditPartyFormValues) => {
    ...
    onPartyUpdate({ 
      ...
      ...partyData, 
      date: combinedDate, 
      menu: values.menu.map(item => item.value) 
    });
    onOpenChange(false);
  };
